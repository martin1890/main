...
// 0 = "action"-region (g�r saker h�r), 1 = "move"-region (flytta soldater hit)
...
void draw_filled_rect(int x, int y, int width, int height, int color)
{
    ...
    if (inner_w < 2 || inner_h < 2)
    {
        ...
        // just fill the last line(s) manually if you vill ha det absolut 100% t�tt,
        // men med draw_rect-logiken ovan �r det redan t�ckt ganska bra.
        return;
    }
...
void init_region_state(void)
{
    ...
    {
        region_state[i][REGION_SOLDIERS] = 1; // alla b�rjar med 1 soldat
...
void move_soldiers_from_action_to_move(
    int turn_player,
    int player_countries[4][15],
    int player_country_counts[4])
{
...
void border_select(int region, int select_type)
{
    ...
    if (select_type == BORDER_SELECT_MOVE) {
        color_select = 220;                 // m�lregion
        prev_region_ptr = &selected_move_region;
    }
    else {
        color_select = 132;                 // "g�r saker i"-region
    ...
    if (*prev_region_ptr >= 1 && *prev_region_ptr <= 30 && *prev_region_ptr != region)
    {
        ...
        // rita border i normal gr�nsf�rg (36) f�r att "ta bort" highlight
        draw_faction_sprite(px, py, pb->data, pb->width, pb->height, 36);
    }

    // Spara nya regionen som vald i den h�r kategorin
    *prev_region_ptr = region;

    // Rita ny selection-border i r�tt f�rg
...
void next_move_target(void)
{
    ...
    // Rita om ACTION-regionens border ovanp� s� den inte "naggas" bort
...
void handle_march_menu_selection(
    int* current_mode,
    int* menu_index,
    int* menu_option_count,
    int* turn_player,
